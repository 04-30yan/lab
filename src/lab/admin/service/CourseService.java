 ...
 * */
	public boolean deleteStudentCourse(Course cos){
		...
		String sqlDelstudentCourse = "delete from studentCourse where id in("+cos.getIds()+")";
		String sqlWork = "select url from work where studentCourseId=?";
		String sqlDelWork = "delete from work where studentCourseId in("+cos.getIds()+")";
	...
	public int addCourse(Course course){
		...
		String sql = "insert into course(courseName,term,userId,addTime) value(?,?,?,?)";
		...
		while(st.hasMoreTokens()){
			...
			try {
				...
				if(courseId.hasMoreTokens() && userId.hasMoreTokens()){
					File dir = new File("E:\\lab\\"+courseId.nextToken()+"\\"+userId.nextToken());
					FileUtil.deleteDir(dir);
				}
				b = true;
			} catch (SQLException e) {
		...
		try {
			...
			for(Course cos:al){
				if(null != cos.getCourseName()){
					ps.setString(1, cos.getCourseName());
					ps.setString(2, cos.getTerm());
					ps.setString(3, "a001");
					ps.setString(4, date);
